...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen eating-training-body">
    ...
    <header class="h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            <div class="p-2 bg-rose-50 rounded-xl">
                <i data-lucide="utensils" class="text-rose-600 w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800 tracking-tight">Persiapan Makan</h2>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- HERO SECTION -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-rose-500 to-pink-600 rounded-[2.5rem] p-10 lg:p-14 text-white shadow-2xl">
                <div class="relative z-10 max-w-2xl">
                    <span class="px-4 py-1.5 bg-white/20 rounded-full text-xs font-bold uppercase">
                        Modul Asesmen
                    </span>
                    <h1 class="text-4xl lg:text-5xl font-black mt-4 mb-4">Kemandirian Makan</h1>
                    <p class="text-rose-50 text-lg">
                        Penilaian persiapan makan anak, mulai dari inisiatif diri, kebersihan tangan, hingga pengambilan porsi makanan secara mandiri.
                    </p>
                </div>
                <i data-lucide="utensils-crossed" class="absolute right-10 top-1/2 -translate-y-1/2 w-48 h-48 text-white/10 rotate-12"></i>
            </div>
        </section>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="persiapanMakanForm">
                    ...
                    <table class="w-full text-sm border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            $aspek_makan = [
                                ...
                                "Duduk di kursi",
                                "Mengambil nasi sesuai porsi",
                                "Mengambil lauk sesuai porsi",
                                "Mengambil sayur sesuai porsi",
                                "Membaca doa sebelum makan"
                            ];
                            $total_rows = count($aspek_makan);

                            foreach($aspek_makan as $index => $a):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap"><?= $a ?></td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_makan[]" 
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionMakan" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Observasi Makan</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 outline-none transition-all"
                              placeholder="Tuliskan catatan terkait adab makan, porsi yang diambil, atau kendala dalam persiapan..."></textarea>
                </div>
...
<script>
    ...
    const formMakan = document.getElementById('persiapanMakanForm');
    
    formMakan.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionMakan();
        }
    });

    function updateTotalConditionMakan() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionMakan');
