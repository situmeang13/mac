...
<style>
    ...
    .week-input:focus {
        border-color: #8b5cf6; /* Warna Violet */
...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen keramas-training-body">
    ...
    <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            <div class="p-2 bg-violet-50 rounded-xl">
                <i data-lucide="droplets" class="text-violet-600 w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800">Keramas</h2>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- Hero Section -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-violet-600 to-purple-700 rounded-[2.5rem] p-10 text-white shadow-xl shadow-violet-200">
                <div class="relative z-10">
                    <h1 class="text-4xl font-black mb-3">Keramas – Kemandirian</h1>
                    <p class="text-violet-50 max-w-2xl text-lg">
                        Penilaian langkah-langkah kemandirian anak dalam mencuci rambut. Pantau perkembangan deskripsi M1-M5 secara berkala.
                    </p>
                </div>
                <i data-lucide="shower-head" class="absolute right-[-20px] bottom-[-20px] w-64 h-64 text-white/10 rotate-12"></i>
            </div>
        </section>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="keramasForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-600 border-b border-slate-200">
                                <th class="p-4 text-left font-bold uppercase tracking-wider">Langkah Keramas</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            $aspek_keramas = [
                                "Mengambil sampo",
                                "Menggosok rambut dengan sampo",
                                "Membilas sampo hingga bersih",
                                "Mematikan Keran/Shower"
                            ];
                            $total_rows = count($aspek_keramas);

                            foreach($aspek_keramas as $index => $a):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap"><?= $a ?></td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_keramas[]" 
                                           class="row-checkbox w-6 h-6 rounded-lg text-violet-600 focus:ring-violet-500 border-slate-300 cursor-pointer">
                                </td>
                                <td class="p-4">
                                    <select class="bg-white border border-slate-200 rounded-lg p-2 text-xs font-medium outline-none w-28 focus:ring-2 focus:ring-violet-500">
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionKeramas" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Tambahan (Keramas)</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-violet-500/10 focus:border-violet-500 outline-none transition-all"
                              placeholder="Tuliskan jika ada kendala atau kemajuan signifikan saat proses keramas..."></textarea>
                </div>
                ...
                <div class="flex justify-end mt-12 gap-4">
                    ...
                    <button type="submit" class="px-12 py-4 bg-violet-600 text-white rounded-2xl font-bold hover:bg-violet-700 shadow-lg shadow-violet-200 transition-all flex items-center gap-2">
...
<script>
    ...
    const formKeramas = document.getElementById('keramasForm');
    
    formKeramas.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionKeramas();
        }
    });

    function updateTotalConditionKeramas() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionKeramas');
