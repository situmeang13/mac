...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen setelah-mandi-body">
    ...
    <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            <div class="p-2 bg-indigo-50 rounded-xl">
                <i data-lucide="check-square" class="text-indigo-600 w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800">Setelah Mandi</h2>
        ...
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[2.5rem] p-10 text-white shadow-xl shadow-indigo-200">
                <div class="relative z-10">
                    <h1 class="text-4xl font-black mb-3">Setelah Mandi – Kemandirian</h1>
                    <p class="text-indigo-50 max-w-2xl text-lg">
                        Penilaian langkah-langkah kemandirian anak setelah selesai mandi. Pantau perkembangan deskripsi M1-M5 secara berkala.
                    </p>
                </div>
                <i data-lucide="sun" class="absolute right-[-20px] bottom-[-20px] w-64 h-64 text-white/10 rotate-12"></i>
            </div>
        </section>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="setelahMandiForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-600 border-b border-slate-200">
                                <th class="p-4 text-left font-bold uppercase tracking-wider">Langkah Setelah Mandi</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            $aspek_setelah_mandi = [
                                "Mengambil Handuk",
                                "Mengeringkan Tubuh",
                                "Memakai pakaian dalam (dalam KM)",
                                "Memakai Baju (dalam KM)",
                                "Memakai Celana (dalam KM)",
                                "Membawa pakaian kotor & handuk",
                                "Keluar kaki kanan",
                                "Doa",
                                "Meletakkan pakaian kotor di tempatnya",
                                "Meletakkan handuk di rak"
                            ];
                            $total_rows = count($aspek_setelah_mandi);

                            foreach($aspek_setelah_mandi as $index => $a):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap"><?= $a ?></td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_setelah_mandi[]" 
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionSetelahMandi" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Tambahan (Setelah Mandi)</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all"
                              placeholder="Tuliskan catatan khusus terkait kerapian atau kendala setelah mandi..."></textarea>
                </div>
...
<script>
    ...
    const formSetelahMandi = document.getElementById('setelahMandiForm');
    
    formSetelahMandi.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionSetelahMandi();
        }
    });

    function updateTotalConditionSetelahMandi() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionSetelahMandi');
