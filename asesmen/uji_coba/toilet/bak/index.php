...
<style>
    ...
    /* Penyesuaian agar tampilan posisi atas terlihat rapi */
    .sticky-condition {
        vertical-align: top; /* Berubah ke top */
...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen toilet-training-body">
    ...
    <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            <div class="p-2 bg-blue-50 rounded-xl">
                <i data-lucide="droplets" class="text-blue-600 w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800">BAK</h2>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- Hero Section -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] p-10 text-white shadow-xl shadow-blue-100">
                <div class="relative z-10">
                    <h1 class="text-4xl font-black mb-3">BAK – Toilet Training</h1>
                    <p class="text-blue-100 max-w-2xl text-lg">
                        Penilaian kemampuan mandiri anak. Isi deskripsi mingguan (M1-M5) dan centang aspek yang telah dikuasai.
                    </p>
                </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="trainingForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-600 border-b border-slate-200">
                                <th class="p-4 text-left font-bold uppercase tracking-wider">Aspek</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php
                            $aspek = [
                                "Sinyal / Code", "Doa Masuk", "Masuk Kaki Kiri", "Lepas Celana",
                                "Gantung Celana", "Pipis di Lubang", "Flush / Menyiram", "Istinja'",
                                "Pakai Celana di KM", "Keluar Kaki Kanan", "Doa Keluar"
                            ];
                            $total_aspek = count($aspek);
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <?php if ($index === 0): ?>
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_aspek ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        <span class="text-[10px] font-black uppercase text-slate-400 tracking-widest leading-tight">Total<br>Terpenuhi</span>
                                        <div id="totalCondition" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Mingguan</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 outline-none transition-all"
                              placeholder="Contoh: Anak sudah mulai inisiatif ke kamar mandi..."></textarea>
                </div>
...
<script>
    ...
    // Logika perhitungan Current Condition
    ...
    function updateTotalCondition() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalCondition');
