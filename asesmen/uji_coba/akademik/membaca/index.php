...
<style>
    ...
    .badge-syllable { background: #e0e7ff; color: #4338ca; }
    .badge-comprehension { background: #f3e8ff; color: #7e22ce; }
</style>

<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen reading-training-body">
    ...
    <header class="h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            <div class="p-2 bg-indigo-50 rounded-xl">
                <i data-lucide="languages" class="text-indigo-600 w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800 tracking-tight">Membaca & Pemahaman</h2>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- HERO SECTION -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2.5rem] p-10 lg:p-14 text-white shadow-2xl">
                <div class="relative z-10 max-w-2xl">
                    <span class="px-4 py-1.5 bg-white/20 rounded-full text-xs font-bold uppercase">
                        Modul Asesmen
                    </span>
                    <h1 class="text-4xl lg:text-5xl font-black mt-4 mb-4">Kemampuan Literasi</h1>
                    <p class="text-indigo-50 text-lg">
                        Pemantauan kemajuan anak mulai dari artikulasi suku kata hingga tingkat pemahaman isi bacaan secara kontekstual.
                    </p>
                </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="membacaForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-600 border-b border-slate-200">
                                <th class="p-4 text-left font-bold uppercase tracking-wider">Tahapan Membaca</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php
                            $aspek_membaca = [
                                ["label" => "Membaca 1 suku kata", "cat" => "syllable"],
                                ["label" => "Membaca 2 suku kata", "cat" => "syllable"],
                                ["label" => "Membaca 3 suku kata", "cat" => "syllable"],
                                ["label" => "Memahami 1 kalimat", "cat" => "comprehension"],
                                ["label" => "Memahami cerpen 3 kalimat", "cat" => "comprehension"],
                                ["label" => "Memahami cerpen 5 kalimat", "cat" => "comprehension"],
                                ["label" => "Memahami cerpen 7 kalimat", "cat" => "comprehension"],
                                ["label" => "Memahami cerpen 10 kalimat", "cat" => "comprehension"]
                            ];
                            
                            $total_rows = count($aspek_membaca);

                            foreach($aspek_membaca as $index => $item):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap">
                                    <span class="level-badge <?= $item['cat'] == 'syllable' ? 'badge-syllable' : 'badge-comprehension' ?>">
                                        <?= $item['cat'] == 'syllable' ? 'Teknis' : 'Pemahaman' ?>
                                    </span><br>
                                    <?= $item['label'] ?>
                                </td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_membaca[]" 
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionMembaca" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        <i data-lucide="message-square-text" class="w-5 h-5 text-indigo-600"></i>
                        <label class="font-bold text-lg">Catatan Observasi Literasi</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all"
                              placeholder="Tuliskan catatan mengenai kecepatan membaca, intonasi, atau kedalaman pemahaman anak terhadap alur cerita..."></textarea>
                </div>
...
<script>
    ...
    const formMembaca = document.getElementById('membacaForm');
    
    formMembaca.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionMembaca();
        }
    });

    function updateTotalConditionMembaca() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionMembaca');
