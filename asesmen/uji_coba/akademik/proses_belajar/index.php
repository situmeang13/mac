...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen learning-process-body">
    ...
    <header class="h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800 tracking-tight">Proses Belajar</h2>
                <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold">Cognitive & Behavioral Development</p>
            </div>
        </div>
    </header>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- HERO SECTION -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[2.5rem] p-10 lg:p-14 text-white shadow-2xl">
                <div class="relative z-10 max-w-2xl">
                    <span class="px-4 py-1.5 bg-white/20 rounded-full text-xs font-bold uppercase">
                        Modul Asesmen
                    </span>
                    <h1 class="text-4xl lg:text-5xl font-black mt-4 mb-4">Kesiapan & Kepatuhan Belajar</h1>
                    <p class="text-indigo-50 text-lg">
                        Pemantauan sikap dasar dalam belajar, mencakup ketenangan, kontak mata, dan regulasi perilaku anak selama sesi instruksional.
                    </p>
                </div>
                <i data-lucide="brain-circuit" class="absolute right-10 top-1/2 -translate-y-1/2 w-48 h-48 text-white/10 rotate-12"></i>
            </div>
        </section>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="prosesBelajarForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-600 border-b border-slate-200">
                                <th class="p-4 text-left font-bold uppercase tracking-wider">Aspek Perilaku Belajar</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            $aspek_belajar = [
                                "Duduk tenang",
                                "Eye contact",
                                "Perilaku ketika belajar"
                            ];
                            $total_rows = count($aspek_belajar);

                            foreach($aspek_belajar as $index => $a):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap"><?= $a ?></td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_belajar[]" 
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionBelajar" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Observasi Belajar</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all"
                              placeholder="Tuliskan catatan terkait durasi konsentrasi, respon terhadap instruksi, atau perilaku spesifik saat sesi belajar berlangsung..."></textarea>
                </div>
...
<script>
    ...
    const formBelajar = document.getElementById('prosesBelajarForm');
    
    formBelajar.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionBelajar();
        }
    });

    function updateTotalConditionBelajar() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionBelajar');
