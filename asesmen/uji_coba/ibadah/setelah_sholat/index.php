...
<div class="flex-grow ml-80 flex flex-col bg-slate-50 min-h-screen spiritual-training-body">
    ...
    <header class="h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800 tracking-tight">Setelah Sholat</h2>
    ...
    <main class="p-8 lg:p-12 flex-grow max-w-[1600px] mx-auto w-full">
        <!-- HERO SECTION -->
        <section class="mb-10">
            <div class="relative overflow-hidden bg-gradient-to-br from-purple-500 to-indigo-600 rounded-[2.5rem] p-10 lg:p-14 text-white shadow-2xl">
                <div class="relative z-10 max-w-2xl">
                    <span class="px-4 py-1.5 bg-white/20 rounded-full text-xs font-bold uppercase">
                        Modul Asesmen
                    </span>
                    <h1 class="text-4xl lg:text-5xl font-black mt-4 mb-4">Aktivitas Setelah Sholat</h1>
                    <p class="text-purple-50 text-lg">
                        Penilaian kemandirian anak dalam merapikan kembali perlengkapan sholat dan menjaga kebersihan tempat ibadah.
                    </p>
                </div>
                <i data-lucide="hands-praying" class="absolute right-10 top-1/2 -translate-y-1/2 w-48 h-48 text-white/10 rotate-12"></i>
            </div>
        </section>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <form class="p-10" id="setelahSholatForm">
                <div class="table-container overflow-x-auto">
                    <table class="w-full text-sm border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php
                            $aspek_setelah = [
                                "Melepas perlengkapan sholat",
                                "Melipat sajadah",
                                "Mengembalikan ke tempatnya"
                            ];
                            $total_rows = count($aspek_setelah);

                            foreach($aspek_setelah as $index => $a):
                            ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 font-semibold text-slate-700 whitespace-nowrap"><?= $a ?></td>
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="score_setelah[]" 
                                           class="row-checkbox w-6 h-6 rounded-lg text-purple-600 focus:ring-purple-500 border-slate-300 cursor-pointer">
                                </td>
                                <td class="p-4">
                                    <select class="bg-white border border-slate-200 rounded-lg p-2 text-xs font-medium outline-none w-28 focus:ring-2 focus:ring-purple-500">
                                ...
                                <td class="p-4 text-center sticky-condition" rowspan="<?= $total_rows ?>">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div id="totalConditionSetelah" class="current-condition-chip condition-empty">0</div>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                ...
                <div class="mt-12">
                    <div class="flex items-center gap-2 mb-4 text-slate-800">
                        ...
                        <label class="font-bold text-lg">Catatan Kedisiplinan & Kerapian</label>
                    </div>
                    <textarea class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 min-h-[120px] focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 outline-none transition-all"
                              placeholder="Tuliskan catatan terkait cara melipat perlengkapan sholat atau ketepatan meletakkan barang..."></textarea>
                </div>
                ...
                <div class="flex justify-end mt-12 gap-4">
                    ...
                    <button type="submit" class="px-12 py-4 bg-purple-600 text-white rounded-2xl font-bold hover:bg-purple-700 shadow-lg shadow-purple-200 transition-all flex items-center gap-2">
...
<script>
    ...
    const formSetelah = document.getElementById('setelahSholatForm');
    
    formSetelah.addEventListener('change', function(e) {
        if (e.target.classList.contains('row-checkbox')) {
            updateTotalConditionSetelah();
        }
    });

    function updateTotalConditionSetelah() {
        const checkboxes = document.querySelectorAll('.row-checkbox');
        const totalDisplay = document.getElementById('totalConditionSetelah');
